...
@implementation doAGIPCGridItem
        ...
        if (_selected != selected)
        {
            if (selected) {
                ...
                if ([self.delegate respondsToSelector:@selector(agGridItemCanSelect:)])
                {
                    ...
                    {
                        NSString *maxStr = [NSString stringWithFormat:@"最多选择%lu张",(unsigned long)numberOfSelectedGridItems];
                        UIAlertView *alterView = [[UIAlertView alloc]initWithTitle:@"提示" message:maxStr delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                        [alterView show];
                        return;
                    }
                }
            ...
            if (self.selected) {
                //[self.checkmarkImageView setImage:[UIImage imageNamed:@"do_Album.bundle/AGIPC-Checkmark-1"] forState:UIControlStateNormal];
                [self.checkmarkImageView setImageWithAnimation:[UIImage imageNamed:@"do_Album.bundle/AGIPC-Checkmark-1"] forState:UIControlStateNormal];
            } else {
                [self.checkmarkImageView setImage:[UIImage imageNamed:@"do_Album.bundle/AGIPC-Checkmark-0"] forState:UIControlStateNormal];
            }
...
- (void)layoutSubviews
{
    ...
    //if (IS_IPAD())
    //self.checkmarkImageView.image = [UIImage imageNamed:@"do_Album.bundle/AGIPC-Checkmark-iPad"];
    //else
    //self.checkmarkImageView.image = [UIImage imageNamed:@"do_Album.bundle/AGIPC-Checkmark-iPhone"];
