...
@interface doYZImageManager ()
@property (nonatomic, strong) ALAssetsLibrary *assetLibrary;
@end

@implementation doYZImageManager

+ (instancetype)manager {
    static doYZImageManager *manager;
...
/// Get Album 获得相册/相册数组
- (void)getCameraRollAlbum:(BOOL)allowPickingVideo completion:(void (^)(doYZAlbumModel *))completion{
    __block doYZAlbumModel *model;
    if (iOS8Later) {
...
- (void)getAllAlbums:(BOOL)allowPickingVideo completion:(void (^)(NSArray<doYZAlbumModel *> *))completion{
...
/// Get Assets 获得照片数组
- (void)getAssetsFromFetchResult:(id)result allowPickingVideo:(BOOL)allowPickingVideo completion:(void (^)(NSArray<doYZAssetModel *> *))completion {
...
- (void)getAssetFromFetchResult:(id)result atIndex:(NSInteger)index allowPickingVideo:(BOOL)allowPickingVideo completion:(void (^)(doYZAssetModel *))completion {
    if ([result isKindOfClass:[PHFetchResult class]]) {
        ...
        NSString *timeLength = type == TZAssetModelMediaTypeVideo ? [NSString stringWithFormat:@"%0.0f",asset.duration] : @"";
        timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
        doYZAssetModel *model = [doYZAssetModel modelWithAsset:asset type:type timeLength:timeLength];
        if (completion) completion(model);
    } else if ([result isKindOfClass:[ALAssetsGroup class]]) {
        ALAssetsGroup *gruop = (ALAssetsGroup *)result;
        if (!allowPickingVideo) [gruop setAssetsFilter:[ALAssetsFilter allPhotos]];
        ...
        [gruop enumerateAssetsAtIndexes:indexSet options:NSEnumerationConcurrent usingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
            doYZAssetModel *model;
            TZAssetModelMediaType type = TZAssetModelMediaTypePhoto;
            if (!allowPickingVideo){
                model = [doYZAssetModel modelWithAsset:result type:type];
                if (completion) completion(model);
                return;
            }
            /// Allow picking video
            if ([[result valueForProperty:ALAssetPropertyType] isEqualToString:ALAssetTypeVideo]) {
                ...
                NSString *timeLength = [NSString stringWithFormat:@"%0.0f",duration];
                timeLength = [self getNewTimeFromDurationSecond:timeLength.integerValue];
                model = [doYZAssetModel modelWithAsset:result type:type timeLength:timeLength];
            } else {
                model = [doYZAssetModel modelWithAsset:result type:type];
            }
            if (completion) completion(model);
...
- (void)getPostImageWithAlbumModel:(doYZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        [[doYZImageManager manager] getPhotoWithAsset:[model.result lastObject] photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
- (doYZAlbumModel *)modelWithResult:(id)result name:(NSString *)name{
    doYZAlbumModel *model = [[doYZAlbumModel alloc] init];
    model.result = result;
