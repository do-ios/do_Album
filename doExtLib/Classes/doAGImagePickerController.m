...
static doAGImagePickerController *_sharedInstance = nil;

@interface doAGImagePickerController ()
{
...
@implementation doAGImagePickerController
...
+ (doAGImagePickerController *)sharedInstance:(id)delegate
{
    if (nil == _sharedInstance){
        @synchronized(self) {
            if (nil == _sharedInstance){
                _sharedInstance  = [doAGImagePickerController imagePickerWithDelegate:nil];
            }
        }
    }
...
+ (doAGImagePickerController *)imagePickerWithDelegate:(id<AGImagePickerControllerDelegate, NSObject>)delegate
{
    doAGImagePickerController *picker = [[doAGImagePickerController alloc] initWithDelegate:delegate];
    
    // Show saved photos on top
    picker.shouldShowSavedPhotosOnTop = NO;
...
andShouldShowSavedPhotosOnTop:(BOOL)shouldShowSavedPhotosOnTop
{
    ...
    if (self)
    {
        ...
        self.viewControllers = @[[[doAGIPCAlbumsController alloc] initWithImagePickerController:self]];
    }
...
{
    doAGIPCAlbumsController *albumsCtl = (doAGIPCAlbumsController *)[self.viewControllers firstObject];
